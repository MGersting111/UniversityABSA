...
BASE_URL = "https://www.studycheck.de/hochschulen/hs-rheinmain/bewertungen"
TITLE = 'hs_rheinmain'
HEADERS = {
...
def collect_review_links_live(max_pages=10):
    ...
    print(f"\n📦 Gesamt: {len(review_urls)} Review-Links")
    return review_urls

if __name__ == "__main__":
    links = collect_review_links_live(max_pages=500)

    with open(f"review_links/review_links_{TITLE}.txt", "w", encoding="utf-8") as f:
        seen = set()
        for link in links:
            if link not in seen:
                f.write(link + "\n")
    ...
    print(f"\n💾 Gespeichert in: review_links_{TITLE}.txt")
